import { Router } from 'express';

import { sendEmailHandler } from '../controllers/email.controller.js';
import { requireAllowedOrigin } from '../middleware/allowedOrigin.js';
import { emailRateLimiter, recipientRateLimiter } from '../middleware/rateLimit.js';
import { uploadAttachments } from '../middleware/upload.js';
import { validate } from '../middleware/validate.js';
import { sendEmailSchema } from '../validators/email.schema.js';

const router = Router();

// origin → rate limit → parse upload → validate → recipient limit → send.
// Upload runs before validate: for multipart requests the text fields only
// exist in req.body once multer has parsed the form.
router.post(
  '/',
  requireAllowedOrigin,
  emailRateLimiter,
  uploadAttachments,
  validate(sendEmailSchema),
  recipientRateLimiter,
  sendEmailHandler,
);

export default router;
